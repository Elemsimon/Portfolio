...
import { productDetails, imageSlider } from "@/data/index";
import MagicButton from './MagicButton';
import { FaLocationArrow } from 'react-icons/fa6';
import ImageCarousel from './ImageCarousel';

const Detail = ({icon,}:{icon:React.ReactNode}) => {
  const path = useParams()
  const url = Number(path.id);
  const data = productDetails.find(user => user.id === url);
        ...
        <div key={data.id} className='relative'>
          <div className='relative flex w-full flex-col justify-between overflow-hidden sm:flex-row gap-24 sm:gap-12 mb-2'>
            <div className='z-20 flex w-full flex-1 flex-col items-start justify-center '>
              <p className="text-start md:tracking-wider mb-4 text-2xl md:text-2xl lg:text-4xl">
                {data.title}
              </p>
            ...
            <div className="flex flex-1 items-center justify-end">
              <img src={data.thumbnail} alt="phones" width={500} height={700} className='object-contain'/>
            </div>
          </div>
          ...
          <div className='relative flex w-full flex-col justify-between overflow-hidden sm:flex-row gap-24 sm:gap-12 my-20'>
            <div className="flex flex-1 items-start justify-end flex-col">
              <h2 className=' ml-10 md:tracking-wider mb-0 text-2xl md:text-2xl lg:text-3xl'>Key Contributions</h2> 
            ...
            <div className="flex flex-1 items-start justify-end flex-col">
              <h2 className=' ml-10 md:tracking-wider mb-0 text-2xl md:text-2xl lg:text-3xl'>Skills Utilized</h2> 
              ...
              <div className='ml-10'>
                <p className="text-start md:tracking-wider mb-6 text-2xl md:text-2xl lg:text-3xl">
                  Outcome
                </p>
                <p className='relative z-20 text-sm md:text-lg leading-[1.6] text-white font-normal'>
                  {data.desc3}
          ...
          <a className='flex justify-center' href={data.link}>
            <MagicButton
              title="Explore Site"
